		...
		runQuery:function (mapExtent, mp) {
			...
			qt.execute(q).addCallback(lang.hitch(this, function (response) {
				...
				if (response.features === undefined) { 
					...
					alert("Something's wrong. It's probably an attribute mismatch. Check the fields.");
				}
				...
				if (nFeatures > 0) {
					...
					array.forEach(response.features, lang.hitch(this, function (feature) {
						...
						//dateCurrent = this.config.MSG_UNKNOWN; //whole thing crashes if no date is assigned - TODO
						startDate = new Date(dateCurrent, 0, 1); 
						
						/*if (this.config.TIMELINE_SCALE === "year") {
							//console.log("in check: " + dateCurrent);  //nonsense
							//startDate = new Date(dateCurrent, month, day);
							startDate = new Date(dateCurrent, 0, 1); //just uses the year now - heather
						...
						var tooltipContent = "<img class='tooltipThumbnail' src='" + this.setThumbnailUrl(this.token, objID) + "'>" +
								...
								"<div class='tooltipHeader'>" + mapName + " (" + fdate + ")</div>" +  
								"<div class='tooltipContent'>" + citation + "<br>1:" + scale + "</div></div>";
		...
		_onSelect:function () {
			...
			if (_sel.length) {
				if (_sel[0].row !== undefined) {
					...
					fdate = _timelineData[_sel[0].row].fdate; //heather
					scl = _timelineData[_sel[0].row].scale,
					whereClause = this.config.IMAGE_SERVER_WHERE + objID;
					mn = _timelineData[row].mapName;
					...
					//console.log(whereClause); // OBJECTID = 5
				...
				} // end of if (_sel[0].row !== undefined) 
			} // end of if (_sel.length)
		}, // end of _onSelect:function () 
